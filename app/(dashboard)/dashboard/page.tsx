...
import { ArrowRight, Sparkles, Zap, VideoIcon, PenTool, Music, Paintbrush } from "lucide-react";
...
export default function HomePage() {
  ...
  useEffect(() => {
    if (activeFilters.length === 0) {
      ...
      setFilteredTools(tools.filter(tool => !tool.professions.includes('all')));
    } else {
  ...
  // Получаем список уникальных профессий для отображения фильтров
  const uniqueProfessions = professions.map(p => p.id as Profession);
    ...
    <div className="space-y-8">
      <div className="space-y-6 relative">
        ...
        <h1 className="text-4xl md:text-6xl font-heading font-bold text-center">
          <span className="bg-clip-text text-transparent bg-gradient-to-r from-indigo-400 via-purple-400 to-pink-400 inline">Creator Studio</span>
        </h1>
        ...
        <div className="flex flex-col items-center">
          <div className="flex items-center gap-2 bg-indigo-900/20 backdrop-blur-sm border border-indigo-500/10 py-3 px-5 rounded-full mb-4">
            ...
            <p className="text-sm text-indigo-300">Elevate your creative content with AI</p>
          </div>
          <p className="text-center text-muted-foreground max-w-[700px] mx-auto text-base md:text-lg">
            Powerful AI tools designed specifically for content creators, video artists, photographers and digital artists.
          </p>
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-4 mt-8">
          {professions.map((profession) => (
            ...
            >
              <profession.icon className={cn("h-5 w-5", profession.iconColor)} />
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-medium px-3 py-1.5 rounded-full bg-indigo-500/10 text-indigo-300 border border-indigo-500/20">
                      <span className="flex items-center gap-1.5">
                        ...
                        {tool.professions.includes('all') ? 'Creator Tool' : 
                          tool.professions.includes('video') ? 'Video Creator' :
                          tool.professions.includes('art') ? 'Digital Artist' :
                          tool.professions.includes('music') ? 'Musician' :
                          'Content Creator'}
                      </span>
                    </span>
